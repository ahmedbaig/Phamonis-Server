...
export class SecureStorageService {
  ...
  setItem(key: string, data: any):boolean{ 
    try{ 
      ...
      let sessionData = AES.encrypt(JSON.stringify(data), sessionKey).toString() 
      if(sessionData != null){
  ...
  getItem(key:string):any{ 
    ...
    let data = sessionData != null ? AES.decrypt(sessionData!, sessionKey).toString(enc.Utf8)  : null
    return data
  }
