...
export class NewUserComponent implements OnInit {
  ...
  age:String = ""
  device:String = ""
  constructor(private _pi:PiService, private secureStorage:SecureStorageService, private _user:UserService, private _auth:AuthServiceService) { }

  ngOnInit() { 
    this._pi.getAll(JSON.parse(this.secureStorage.getItem('session_t')).jwt).subscribe(res=>{
      this.devices = filter(res.devices, o=>{return o.user == null})
    })
    delay(()=>{
      M.AutoInit();
      $('select').formSelect();
