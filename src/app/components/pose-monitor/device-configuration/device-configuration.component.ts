...
export class DeviceConfigurationComponent implements OnInit {
  ...
  save(){
    ...
    let body = {model: this.model, threshold: this.threshold}
    this._piService.updateDeviceUser(body, JSON.parse(this.secureStorage.getItem('session_t')).jwt, this.id).subscribe(res=>{
      if(res.success){
