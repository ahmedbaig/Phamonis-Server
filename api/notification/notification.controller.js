...
exports.readAll = async function(req, res) {
    try {
        await NotificationModel.findOne({ user: req.params.user, read: true })
            .exec(function(err, doc) {
                // sending access notification
                console.log(doc)
                let read = _.filter(doc, { read: true })
                let unread = _.filter(doc, { read: false })
...
exports.read = async function(req, res) {
    try {
        await NotificationModel.findOneAndUpdate({ user: req.params.user, _id: req.params.notification }, { read: true })
            .then(async function() {
                await NotificationModel.find({ user: req.params.user, read: true }).exec((err, doc) => {
                    // sending access notification
